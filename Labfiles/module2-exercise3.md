...
4. In the **redhat** remote desktop session, enable the root user by running the below commands, and enter the password as **demo!pass123**. 

    ```
    su 
    ```
   
5. Once you connect with the root user, navigate to the **\\etc\\www\\html\\inetpub\\SmartHotel.Registration** folder by running the below commands.
...
6. Update the **DefaultConnection** setting to connect to your Azure SQL Database.
   ...
   You can find the connection string for the Azure SQL Database in the Azure portal. Navigate to the **SmartHotelRG** resource group, and then to the database **smarthoteldb** and from the overview, select **Show database connection strings**.
    ...
    To edit in the Vim editor, press **i** then copy the **ADO.NET** connection string, and paste it into the web.config file on **redhat**, replacing the existing connection string.  **Be careful not to overwrite the 'providerName' parameter which is specified after the connection string.** 
