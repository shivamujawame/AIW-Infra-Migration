...
1. Return to the **Azure Migrate | Servers, databases and web apps** blade in the Azure Portal, and select **Servers, databases and web apps (1)** under **Migration goals** on the left. 
...
13. In the discovered list, you'll see the **RedhatVM**, we will be replicating this Redhat VM in the next exercise with the help of Azure Migrate.
...
4. The **Virtual machines** tab should now show the virtual machines included in the assessment. Select the **RedhatVM** virtual machine, then select **Next**.

   ![Screenshot of the 'Virtual machines' tab of the 'Replicate' wizard in Azure Migrate Server Migration. The UbuntuWAF, smarthotelweb1, and smarthotelweb2 machines are selected.](Images/migrateredhat.png "Replicate - Virtual machines")
...
1. Still using the **Migration and modernization - Replicating machines** blade, select the **Redhat VM** virtual machine. This opens a detailed migration and replication blade for this machine. Take a moment to study this information.
...
6. Select **OK** to close the network interface settings blade, then **Save** the **RedhatVM** settings to configure the private IP address for the VMs.
...
2. On the **Migrate** blade, select **yes** for **Shutdown machines before migration to minimum data loss** and select the **RedhatVM** virtual machine then select **Migrate** to start the migration process.

   ![Screenshot of the 'Migrate' blade, with 3 machines selected and the 'Migrate' button highlighted.](Images/starmigrationredhat.png "Migrate - VM selection")
...
6. Navigate to the **SmartHotelHostRG** resource group and check that the VM, network interface, and disk resources have been created for each of the virtual machine being migrated.

   ![Screenshot showing resources created by the test failover (VMs, disks, and network interfaces).](Images/redhatrg.png "Migrated resources")
...
The application tier machine **RedhatVM** is configured to connect to the application database running on-Prem.

On the migrated VM **RedhatVM**, this configuration needs to be updated to use the Azure SQL Database instead.
...
2. Click on **RedhatVM** VM, from the overview blade, and select **Connect**. Select **Bastion** from the available options and click on **Use Bastion**.
...
In this task, you updated the **RedhatVM** configuration to connect to the Azure SQL Database.
...
1. Set the Target type to **Virtual machine** and the Target to the NIC of **RedhatVM**; select **Save** to update the backend pool
